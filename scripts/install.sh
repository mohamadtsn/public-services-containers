...
if [[ "$IS_UPDATE" == "true" ]]; then
    ...
    echo -e "       ${COLOR_GREEN}pubservices run make up${COLOR_RESET}           MySQL + Redis only"
    echo -e "       ${COLOR_GREEN}pubservices run make up-proxy${COLOR_RESET}     + Nginx"
    echo -e "       ${COLOR_GREEN}pubservices run make up-full${COLOR_RESET}      Everything"
    echo ""
